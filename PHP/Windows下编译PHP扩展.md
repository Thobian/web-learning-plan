...
于是想从另一种方式入手：分析代码性能，找出具体什么地方慢，但不巧的是PHP常用的一个性能分析工具 [xhprof ](https://www.php.net/manual/zh/book.xhprof.php) 并没有我需要的扩展版本（7.0.25），就这样走上了编译PHP Windows扩展之路。
...
> 由于php-sdk存在PHP的版本差异，而我本地环境是7.0.25，这部分内容并不适用于PHP7.2及以上版本，不过PHP7.2及以上版本的编译也基本大同小异。PHP7.2及以上版本可以参考官方文档：https://wiki.php.net/internals/windows/stepbystepbuild_sdk_2
...
更多关于编译过程中 `INCLUDE` 、 `LIBPAHT` 两个环境变量的说明可以查看这篇文章：//todo 文章链接。废话不多说，下面进入正题。
...
# --disable-all 禁用所有扩展
# --disable-zts 不启用线程安全，一般线程安全模式只有PHP跟IIS搭配是需要启用 //todo 确认下是否真的这样
